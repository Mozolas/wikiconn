...
export function PlayersList({ players, hostPlayerId, selfPlayerId }: Props): ReactNode {
  if (players.length === 0) {
    return <p className="py-2 text-sm text-muted-foreground">Waiting for players to join…</p>;
  }
      ...
      {players.map((player, index) => (
        ...
        >
          ...
          {player.connected ? null : (
            <WifiOff aria-label="Disconnected" className="size-3.5 text-muted-foreground" />
          )}
